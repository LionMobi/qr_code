...
public class QRCodeUtil {
    ...
    public static Bitmap createQRCodeBitmap(String content, int width, int height){
        return createQRCodeBitmap(content, width, height, "UTF-8", "H", "1", 0xffff0000, 0xffffff00);
    }
            ...
            for(int y = 0; y < height; y++){
                for(int x = 0; x < width; x++){
                    if(bitMatrix.get(x, y)){
                        int x2 = x-width/2;
                        int y2 = y-height/2;
//                        if((x2*x2+y2*y2-1)*(x2*x2+y2*y2-1)*(x2*x2+y2*y2-1)+x2*x2*y2*y2*y2 <0){
                            if(x2*x2 + y2*y2 < width/2.2*height/2.2){
